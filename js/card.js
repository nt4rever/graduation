...
export const card = (() => {
    ...
    return `
      ...
      "dark"
            ...
            <div class="d-flex justify-content-start align-items-center">
                <p class="my-0 mx-1" data-count-like="${comment.like.love}">${
      comment.like.love
    } like</p>
                <i class="me-1 ${
  ...
  const renderAction = (comment) => {
    ...
    if (
      ...
      action += `<button style="font-size: 0.8rem;" onclick="comment.reply(this)" data-uuid="${
        comment.uuid
      }" class="btn btn-sm btn-outline-${theme.isDarkMode(
        "light",
        "dark"
    ...
    if (
      ...
      action += `<button style="font-size: 0.8rem;" onclick="comment.edit(this)" data-uuid="${
        comment.uuid
      }" class="btn btn-sm btn-outline-${theme.isDarkMode(
        "light",
        "dark"
    ...
    if (session.get("token")?.split(".").length === 3) {
      action += `<button style="font-size: 0.8rem;" onclick="comment.remove(this)" data-uuid="${
        comment.uuid
      }" class="btn btn-sm btn-outline-${theme.isDarkMode(
        "light",
        "dark"
      )} rounded-3 py-0" data-own="${comment.own}">Delete</button>`;
    } else if (
      owns.has(comment.uuid) &&
    ...
    return `
        <div class="p-2 my-2 rounded-3 border">
            <p class="text-${theme.isDarkMode(
              "light",
              "dark"
            )} mb-1 mx-0 mt-0 p-0" style="font-size: 0.7rem;" id="ip-${
      comment.uuid
